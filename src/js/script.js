...
/*activitiy retrival from completed tasks*/
let activities =
JSON.parse(
...
function updateAnalytics(){
    ...
    pending;
    dashboardProductivity.textContent =
    productivity + '%';
    progressCircle.style.background =
    `conic-gradient(
    #3b82f6 0%,
    #3b82f6 ${productivity}%,
    rgba(255,255,255,0.1) ${productivity}%
        )`;
    document.querySelector(
'.inner-circle'
).textContent =
...
function(index){
    ...
    if(tasks[index].completed){
        addActivity(
            `✅ Completed: ${tasks[index].text}`
        );
    }else{
        addActivity(
            `↩️ Reopened: ${tasks[index].text}`
        );
    }
    renderTasks();
};
